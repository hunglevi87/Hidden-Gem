...
import { ebayMcpRouter } from "./ebay-mcp-router";
import {
  updateEbayListing,
  deleteEbayListing,
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!userId || isNaN(id)) {
          return res
            .status(400)
            .json({ error: "userId and notification id are required" });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!userId || isNaN(id)) {
          return res
            .status(400)
            .json({ error: "userId and stash item id are required" });
        }
  ...
  app.put("/api/settings/threshold", async (req: Request, res: Response) => {
    try {
      ...
      if (!userId || threshold === undefined) {
        return res
          .status(400)
          .json({ error: "userId and threshold are required" });
      }
      ...
      if (existing) {
        ...
        await db
          .insert(userSettings)
          .values({ userId, highValueThreshold: threshold });
      }
  ...
  app.post(
    "/api/stash/:id/publish/woocommerce",
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        const { storeUrl, consumerKey, consumerSecret, skipThresholdCheck } =
          req.body;

        if (!storeUrl || !consumerKey || !consumerSecret) {
          return res
            .status(400)
            .json({ error: "Missing WooCommerce credentials" });
        }
        ...
        if (item.publishedToWoocommerce) {
          return res
            .status(400)
            .json({ error: "Item already published to WooCommerce" });
        }
        ...
        if (!skipThresholdCheck) {
          ...
          if (suggestedPrice) {
            ...
            if (
              suggestedPrice > threshold &&
              item.publishStatus !== "approved"
            ) {
              await db
                .update(stashItems)
                .set({
                  publishStatus: "held_for_review",
                  updatedAt: new Date(),
                })
                .where(eq(stashItems.id, id));
        ...
        const priceMatch = item.estimatedValue?.match(
          /\$(\d+(?:,\d{3})*(?:\.\d{2})?)/,
        );
        ...
        if (!wooResponse.ok) {
          ...
          return res.status(wooResponse.status).json({
            error:
              errorData.message || `WooCommerce error: ${wooResponse.status}`,
          });
        }
      ...
      try {
        ...
        if (!refreshToken) {
          return res.status(400).json({
            error:
              "User OAuth refresh token required to create listings. Generate a refresh token from eBay Developer Portal and add it in eBay settings.",
          });
        }
        ...
        if (item.publishedToEbay) {
          return res
            .status(400)
            .json({ error: "Item already published to eBay" });
        }
        ...
        if (!tokenResponse.ok) {
          ...
          return res.status(tokenResponse.status).json({
            error:
              ...
              "Failed to authenticate with eBay. Check your credentials and refresh token.",
          });
        }
        ...
        if (!inventoryResponse.ok && inventoryResponse.status !== 204) {
          ...
          const errorMessage =
            ...
            `eBay inventory error: ${inventoryResponse.status}`;
        ...
        if (offerResponse.ok || offerResponse.status === 201) {
          ...
          const requiresPolicies = offerError.errors?.some(
            (e: any) => e.message?.includes("policy") || e.errorId === 25002,
          );

          if (requiresPolicies) {
            return res.status(400).json({
              error:
                "eBay requires business policies (shipping, payment, return) to be configured in your Seller Hub before listing items. Please set up these policies at ebay.com/sh/settings/policies.",
            });
          }

          return res.status(offerResponse.status).json({
            error:
              offerError.errors?.[0]?.message ||
              `eBay offer error: ${offerResponse.status}`,
          });
        }
        ...
        res.json({
          ...
          message: listingId
            ? "Item listed on eBay"
            : "Inventory created. Check your Seller Hub to complete the listing.",
        });
      } catch (error) {
        console.error("eBay publish error:", error);
        res.status(500).json({
          error:
            "Failed to publish to eBay. Please check your credentials and try again.",
        });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const { previousResult, feedback, provider, apiKey, endpoint, model } =
          req.body;

        if (!previousResult || !feedback) {
          return res
            .status(400)
            .json({ error: "previousResult and feedback are required" });
        }
        ...
        const primaryConfig: AIProviderConfig = {
          provider: (requestedProvider ||
            "openfang") as AIProviderConfig["provider"],
          apiKey,
          endpoint,
          model,
        };

        const parsedPrevious: AnalysisResult =
          typeof previousResult === "string"
            ? JSON.parse(previousResult)
            : previousResult;

        try {
          const result = await analyzeItemWithRetry(
            primaryConfig,
            images,
            parsedPrevious,
            feedback,
          ...
          const fallbackResult = await analyzeItemWithRetry(
            fallbackConfig,
            images,
            parsedPrevious,
            feedback,
  ...
  app.post("/api/seo/generate", async (req: Request, res: Response) => {
    try {
      ...
      if (sellerId && imageUrl) {
        aiRecordId = await createAIRecord({
          sellerId,
          productId,
          imageUrl,
          analysis,
  ...
  app.put("/api/ebay/listing/:itemId", async (req: Request, res: Response) => {
    try {
      const { clientId, clientSecret, refreshToken, environment, ...input } =
        req.body;
