...
export async function getWooCommerceSettings(): Promise<WooCommerceSettings | null> {
  try {
    ...
    if (Platform.OS !== "web") {
      ...
      consumerSecret = await SecureStore.getItemAsync(
        "woocommerce_consumer_secret",
      );
    } else {
      consumerKey = await AsyncStorage.getItem("woocommerce_consumer_key");
      consumerSecret = await AsyncStorage.getItem(
        "woocommerce_consumer_secret",
      );
    }
...
export async function getEbaySettings(): Promise<EbaySettings | null> {
  try {
    ...
    return {
      ...
      environment: (env === "production" ? "production" : "sandbox") as
        | "sandbox"
        | "production",
    };
  } catch (error) {
...
export async function publishToWooCommerce(
  ...
  try {
    const res = await apiRequest(
      "POST",
      `/api/stash/${itemId}/publish/woocommerce`,
      {
    ...
    return {
      success: false,
      error: error.message || "Failed to publish to WooCommerce",
    };
  }
}
